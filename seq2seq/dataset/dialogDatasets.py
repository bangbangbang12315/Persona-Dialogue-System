...
class TranslateData():
    ...
    def pad_his(self, his, pad_len=15):
        len_his = len(his)
        padding = [0.0] * 100
        for i in range(pad_len - len_his):
            his.append(copy.deepcopy(padding))
        return his

    def collate_fn(self, batch):
        src = list(map(lambda x: x['src'], batch))
        tgt = list(map(lambda x: x['tgt'], batch))
        user = list(map(lambda x: x['user'], batch))
        his = list(map(lambda x: x['his'], batch))
        query = list(map(lambda x: x['query'], batch))
        ...
        his = torch.stack(his)
        user = torch.stack(user)
        query = torch.stack(query)
        src_len = torch.stack(src_len)
        tgt_len = torch.stack(tgt_len)
        return {'src': src, 'tgt': tgt, 'user': user, 'his': his, 'query': query, 'src_len': src_len, 'tgt_len': tgt_len}
    ...
    def translate_data(self, subs, obj):
        ...
        for h in his:
            h = h.split(' ')
            h = list(map(lambda x: float(x), h))
            his_list.append(h)
        query = list(map(lambda x: float(x), query))
        ...
        user_ids = [obj.user_vocab.word2idx[user]]
        his_list = self.pad_his(his_list)
        return {"src": torch.LongTensor(src_ids), 
                "tgt": torch.LongTensor(tgt_ids), 
                "user": torch.LongTensor(user_ids),
                "his": torch.FloatTensor(his_list),
...
class DialogDataset(Dataset):
    def __init__(self, src_data_fp, tgt_data_fp, user_data_fp, his_data_fp, query_data_fp, transform_fuc, src_vocab, tgt_vocab, user_vocab, max_src_length, max_tgt_length):
        self.datasets = []
        ...
        loaded = 0
        data_monitor = 0
        with open(src_data_fp, 'r') as f_src, open(tgt_data_fp, 'r') as f_tgt, open(user_data_fp, 'r') as f_user, open(his_data_fp, 'r') as f_his, open(query_data_fp, 'r') as f_query:
            for line_src, line_tgt, line_user, line_his, line_query in tqdm(zip(f_src, f_tgt, f_user, f_his, f_query), desc="Load Data: "):
                src = line_src.strip()
                tgt = line_tgt.strip()
                user = line_user.strip()
                his = line_his.strip()
                query = line_query.strip()
                subs = [src, tgt, user, his, query]
                loaded += 1
