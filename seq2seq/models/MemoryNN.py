...
class MemoryNN(nn.Module):
    ...
    def init_weight(self):
        components = [self.gate, self.attention_grucell,
                     self.memory_grucell]
    ...
    def forward(self, allhis, encoded_questions, n_episode=3):
        bsize = allhis.size(0)
        n_his = allhis.size(1)
        # size = allhis.size(2)
        # encoded_his = []
        ...
        encoded_his = allhis
        device = encoded_questions.device if encoded_questions is not None else torch.device('cpu')
        for i in range(n_episode):
            ...
            # [nhis, b, h]
            encoded_his_t = encoded_his.transpose(0, 1)
